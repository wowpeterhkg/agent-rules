...
description: Use when changing code that is expensive to change safely because behavior is unclear, tests are weak or missing, dependencies are hidden, or runtime/framework setup blocks local feedback. Do NOT use for unrelated work, and never load a second book-* skill in the same task.
---

# Working with legacy code

> Distilled from *Working Effectively with Legacy Code by Michael Feathers*. MIT-licensed ruleset vendored from
...
- **`book-refactoring`** — Once a green suite exists, that skill takes over.
