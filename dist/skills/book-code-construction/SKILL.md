---
name: book-code-construction
description: Use when implementing, changing, reviewing, debugging, refactoring, or tuning production code where construction discipline must reduce defects and keep code easy to inspect. Do NOT use for unrelated work, and never load a second book-* skill in the same task.
---

# Code construction

> Distilled from *Code Complete by Steve McConnell*. MIT-licensed ruleset vendored from
...
- **`any more specific book-* skill`** — This one loses. It is the fallback when nothing else matches.
